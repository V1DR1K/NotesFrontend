...
export function DashboardView({ onNavigate }: { onNavigate: (section: SectionKey) => void }) {
  ...
  const activities = [
    ...
    data?.recentMovements?.[0] ? { section: "finances" as const, icon: "$", title: "Movimiento registrado", detail: `${data.recentMovements[0].conceptCode} · ${dateLabel(data.recentMovements[0].date, true)}` } : null,
