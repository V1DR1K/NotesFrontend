...
export function DashboardView({ onNavigate }: { onNavigate: (section: SectionKey) => void }) {
  ...
  const activities = [
    recentNote ? { section: "notes" as const, icon: "✎", title: "Nota actualizada", detail: `${recentNote.title} · ${dateLabel(recentNote.date, true)}` } : null,
    data?.recentMovements?.[0] ? { section: "finances" as const, icon: "$", title: "Movimiento registrado", detail: `${data.recentMovements[0].item?.label ?? data.recentMovements[0].itemCode} · ${dateLabel(data.recentMovements[0].date, true)}` } : null,
    data?.recentFiles?.[0] ? { section: "files" as const, icon: "↗", title: "Archivo agregado", detail: `${data.recentFiles[0].name} · ${dateLabel(data.recentFiles[0].uploadedAt, true)}` } : null,
  ...
  return <div className="view view-overview">
    ...
    {loading ? <SkeletonGrid count={4} /> : error ? <ErrorState onRetry={reload} /> : <>
      <section className="overview-section"><div className="section-heading-row"><div><span className="eyebrow">TUS ESPACIOS</span><h2>Todo en su lugar.</h2></div><span className="heading-meta">04 MÓDULOS ACTIVOS <span>·</span> DATOS REALES</span></div><div className="module-grid">
        {[{ section: "day" as const, count: counter("days", "dayEntries", "dayCount"), detail: data?.recentDays?.[0] ? `Último: ${dateLabel(data.recentDays[0].date)}` : "Sin registros todavía", emoji: "☀", accentText: "Estado de ánimo" }, { section: "finances" as const, count: cash !== undefined ? formatARS(cash) : counter("movements", "movementCount"), detail: "Caja disponible", emoji: "$", accentText: "Este mes" }, { section: "files" as const, count: counter("files", "fileCount"), detail: data?.recentFiles?.[0]?.name ?? "Sin archivos todavía", emoji: "↗", accentText: "Repositorio" }, { section: "notes" as const, count: counter("notes", "noteCount"), detail: data?.recentNotes?.[0]?.title ?? "Sin notas todavía", emoji: "✎", accentText: "Pensamientos" }].map((card) => <button className="module-card" type="button" onClick={() => onNavigate(card.section)} key={card.section}><div className="module-card-top"><VisualTile emoji={card.emoji} label={card.section} /><span className="module-card-arrow">↗</span></div><div className="module-card-copy"><span className="eyebrow">{card.accentText}</span><h3>{card.section === "day" ? "Mi día" : card.section === "finances" ? "Finanzas" : card.section === "files" ? "Archivos" : "Notas"}</h3><p>{card.detail}</p></div><div className="module-card-bottom"><strong>{String(card.count)}</strong><span>{card.accentText}</span></div></button>)}
      </div></section>
