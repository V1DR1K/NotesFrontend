  ...
  return <div className="view view-overview">
    ...
    {loading ? <SkeletonGrid count={4} /> : error ? <ErrorState onRetry={reload} /> : <>
      <section className="dashboard-signal-grid" aria-label="Resumen de hoy">
        ...
        <button type="button" className="dashboard-signal dashboard-signal-tasks" onClick={() => onNavigate("tasks")}><span className="eyebrow">TAREAS ABIERTAS</span><strong>{(taskStats?.pending ?? 0) + (taskStats?.inProgress ?? 0)}</strong><span>{taskStats?.overdue ? `${taskStats.overdue} vencida${taskStats.overdue === 1 ? "" : "s"}` : "Todo bajo control"}</span><span className="dashboard-signal-icon">✓</span></button>
      </section>

      <section className="dashboard-panel dashboard-task-panel"><div className="panel-heading"><div><span className="eyebrow">TAREAS / PRÓXIMOS PASOS</span><h2>Lo que sigue.</h2></div><button type="button" className="panel-link" onClick={() => onNavigate("tasks")}>Ver tablero ↗</button></div>{upcomingTasks.length ? <div className="dashboard-task-list">{upcomingTasks.map((task) => <button type="button" className={`dashboard-task-row ${task.dueDate && task.status !== "COMPLETED" && task.dueDate < new Date().toISOString().slice(0, 10) ? "dashboard-task-row-overdue" : ""}`} key={task.id} onClick={() => onNavigate("tasks")}><span className="dashboard-task-status">{task.status === "IN_PROGRESS" ? "→" : task.status === "COMPLETED" ? "✓" : "○"}</span><span className="dashboard-task-copy"><strong>{task.title}</strong><small>{task.category.label} · {task.dueDate ? dateLabel(task.dueDate) : "Sin fecha límite"}</small></span><span className="activity-arrow">→</span></button>)}</div> : <p className="dashboard-empty-copy">No hay tareas abiertas. Podés crear la próxima desde el tablero.</p>}<Button variant="ghost" onClick={() => onNavigate("tasks")}>Crear tarea <span aria-hidden="true">↗</span></Button></section>
      ...
      <section className="dashboard-lower-grid">
        ...
        <article className="dashboard-panel dashboard-activity-panel"><div className="panel-heading"><div><span className="eyebrow">ACTIVIDAD RECIENTE</span><h2>Lo último.</h2></div><span className="panel-index">02</span></div>{activities.length ? <div className="activity-list">{activities.slice(0, 6).map((activity) => <button type="button" onClick={() => onNavigate(activity.section)} key={`${activity.section}-${activity.id}`}><span className="activity-icon">{activityIcon(activity.section)}</span><span className="activity-copy"><strong>{activity.title}</strong><small>{activity.detail}</small></span><span className="activity-date">{activity.date ? dateLabel(activity.date) : "—"}</span><span className="activity-arrow">→</span></button>)}</div> : <p className="activity-empty">Todavía no hay actividad reciente.</p>}</article>
      </section>

      <section className="dashboard-spaces"><div className="section-heading-row"><div><span className="eyebrow">TUS SEIS ESPACIOS</span><h2>Todo en su lugar.</h2></div><span className="heading-meta">DATOS REALES <span>·</span> ACCESOS RÁPIDOS</span></div><div className="dashboard-space-grid">
        ...
        <button type="button" className="dashboard-space-card dashboard-space-tasks" onClick={() => onNavigate("tasks")}><VisualTile emoji={SECTION_META.tasks.icon} label={SECTION_META.tasks.label} /><span><small>TAREAS</small><strong>{(taskStats?.pending ?? 0) + (taskStats?.inProgress ?? 0)}</strong><em>{taskStats?.overdue ? `${taskStats.overdue} vencida${taskStats.overdue === 1 ? "" : "s"}` : "pendientes abiertas"}</em></span><b>↗</b></button>
