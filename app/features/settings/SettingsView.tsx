...
import type { ApiConfig, ApiOption, ConfigKind } from "../../lib/api/types";
...
const emptyDraft = (): Draft => ({ code: "", label: "", emoji: "", sortOrder: "0", active: true });
...
export function SettingsView({ config, onConfigChanged }: { config: ApiConfig; onConfigChanged: (config: ApiConfig) => void }) {
  ...
  const openEdit = (group: ConfigGroup, option: ApiOption) => { setError(""); setDraft({ code: option.code, label: option.label, emoji: option.emoji ?? "", sortOrder: String(option.sortOrder ?? 0), active: option.active !== false }); setEditing({ group, option }); };
  ...
  const save = async () => {
    ...
    setError("");
    try {
      if (editing.option) await api.updateConfigOption(editing.group.kind, editing.option.code, { label: draft.label.trim(), emoji: editing.group.emoji ? draft.emoji.trim() : undefined, sortOrder: Number(draft.sortOrder) || 0, active: draft.active });
      else await api.createConfigOption(editing.group.kind, { code: draft.code.trim().toLowerCase().replace(/\s+/g, "_"), label: draft.label.trim(), emoji: editing.group.emoji ? draft.emoji.trim() : undefined, sortOrder: Number(draft.sortOrder) || 0, active: draft.active });
