...
export function FinanceAccountsPanel({ accounts, rate, onSync, onTransfer, onInvest }: { accounts: FinanceAccount[]; rate: number; onSync: (account: FinanceAccount) => void; onTransfer: () => void; onInvest: () => void }) {
  return <section className="finance-accounts" aria-labelledby="finance-accounts-title">
    ...
    {accounts.length ? <div className="finance-account-grid">{accounts.map((account) => <article className={`finance-account finance-account-${account.type.toLowerCase()}`} key={account.code}><div className="finance-account-top"><span className="finance-account-kind">{accountKind(account.type)}</span><span className="finance-account-dot" aria-hidden="true" /></div><h3>{account.label}</h3><strong>{formatARS(account.balanceArs)}</strong>{rate ? <span className="finance-account-usd">{formatUSD(asNumber(account.balanceArs) / rate)} en referencia USD</span> : null}<div className="finance-account-meta">{account.growthMode === "DAILY_TNA" ? <span>+ {asNumber(account.annualRatePercent).toLocaleString("es-AR")} % TNA diaria</span> : <span>Sin valuación automática</span>}<div className="finance-account-actions">{account.type === "CASH" ? <Button variant="quiet" onClick={onTransfer}>Pasar a Cripto</Button> : account.type === "CRYPTO" ? <Button variant="quiet" onClick={onInvest}>Invertir</Button> : null}<Button variant="quiet" onClick={() => onSync(account)}>Actualizar saldo</Button></div></div></article>)}</div> : <p className="analytics-empty">No hay cuentas configuradas.</p>}
  </section>;
}
