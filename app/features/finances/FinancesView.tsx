...
export function FinancesView({ config, focusId }: { config: ApiConfig; focusId?: string | null }) {
  ...
  const save = async () => {
    ...
    setAmountError("");
    try {
      const body = { date: draft.date, bucket: draft.bucket, accountCode: draft.accountCode, itemCode: draft.itemCode, amountArs: amount, note: draft.note.trim() || undefined };
  ...
  const openSync = (account: FinanceAccount) => {
    ...
    setSyncBalance(formatARSInputNumber(account.balanceArs));
    setSyncBalanceError("");
    mutation.clearError();
  };

  const syncAccount = async () => {
    const amount = parseARSInput(syncBalance);
    ...
    setSyncing(true);
    try {
      await mutation.run(() => api.syncFinanceAccount(syncingAccount.code, { balanceArs: amount }));
  ...
  const resetFilters = () => {
   ...
   return <div className="view module-view">
     ...
     <CryptoInvestmentPanel summary={cryptoSummary} onInvest={startInvestment} onSell={startSale} onCompletePrice={startLegacyPrice} onVoidPurchase={(investment) => { mutation.clearError(); setPendingCryptoVoid({ kind: "purchase", investmentId: investment.id, label: investment.assetLabel }); }} onVoidSale={(investment, sale) => { mutation.clearError(); setPendingCryptoVoid({ kind: "sale", investmentId: investment.id, saleId: sale.id, label: investment.assetLabel }); }} />
