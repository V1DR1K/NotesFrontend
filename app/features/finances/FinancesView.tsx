...
const cashIncomeItems = new Set(["sueldo", "otro"]);
const cashExpenseItems = new Set(["pedidos_ya", "comida_afuera", "supermercado", "nafta", "uber_didi"]);
...
function financeItemOptions(accountCode: string, bucket: string, accounts: FinanceAccount[], items: ApiConfig["financeItems"]) { const account = accounts.find((candidate) => candidate.code.toLowerCase() === accountCode.toLowerCase()); const codes = !account || account.type === "CASH" ? bucket === "INCOME" ? cashIncomeItems : cashExpenseItems : new Set(["transferencia"]); return items.filter((item) => codes.has(item.code.toLowerCase())); }
...
export function FinancesView({ config }: { config: ApiConfig }) {
  ...
  const [movements, summary, analytics, ratePayload, accounts] = data.data ?? [null, null, null, null, []];
