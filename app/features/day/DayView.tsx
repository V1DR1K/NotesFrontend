...
export function DayView({ config }: { config: ApiConfig }) {
  ...
  const startNew = () => { setEditingId(null); setDraft({ date: todayIso(), statusCode: config.dayStatuses[0]?.code ?? "", feelings: [], description: "" }); mutation.clearError(); setComposerOpen(true); };
  const startEdit = (entry: NonNullable<typeof data.data>["content"][number]) => { setEditingId(entry.id); setDraft({ date: entry.date, statusCode: entry.statusCode, feelings: parseFeelings(entry.feeling), description: entry.description }); mutation.clearError(); setComposerOpen(true); };
  const save = async () => {
