...
export function FilesView() {
  ...
  const data = useFilesData(page, kind, folderId);
  ...
  return <div className="view module-view">
    ...
    <input ref={fileInputRef} className="visually-hidden" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleFile(file); event.target.value = ""; }} />
    {uploadComposerOpen ? <Dialog ariaLabel="Subir archivo" onClose={() => setUploadComposerOpen(false)}><FormPanel title="Subir archivo" description="El archivo se guarda en el backend como multipart, junto con la carpeta elegida." onClose={() => setUploadComposerOpen(false)}><div className="form-grid"><SelectField label="Carpeta" id="upload-folder" value={uploadFolderId} onChange={setUploadFolderId} options={[{ value: "", label: "Sin carpeta" }, ...(folders?.content ?? []).map((folder) => ({ value: folder.id, label: folder.name }))]} /><div className="upload-modal-zone"><div className="dropzone" onClick={() => fileInputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files?.[0]; if (file) void handleFile(file); }} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") fileInputRef.current?.click(); }}><span className="dropzone-icon">↑</span><span><strong>Soltá un archivo acá</strong><small>o hacé clic para buscarlo en tu dispositivo</small></span><span className="dropzone-meta">MULTIPART READY</span></div></div></div>{mutation.error ? <div className="inline-error" role="alert" aria-live="polite">{mutation.error.message || fieldError(mutation.error, "file", "folderId")}</div> : null}<div className="form-actions"><Button variant="quiet" onClick={() => setUploadComposerOpen(false)}>Cerrar</Button><Button variant="ghost" onClick={() => fileInputRef.current?.click()}>Elegir archivo</Button></div></FormPanel></Dialog> : null}
    ...
    {renameId ? <Dialog ariaLabel="Renombrar archivo" onClose={() => setRenameId(null)}><FormPanel title="Renombrar archivo" description="El nuevo nombre se guardará en los metadatos del archivo." onClose={() => setRenameId(null)}><div className="form-grid"><FormField label="Nombre" value={renameValue} onChange={setRenameValue} /></div>{mutation.error ? <div className="inline-error" role="alert" aria-live="polite">{mutation.error.message}</div> : null}<div className="form-actions"><Button variant="quiet" onClick={() => setRenameId(null)}>Cancelar</Button><Button onClick={() => void saveRename()} disabled={!renameValue.trim()}>Guardar nombre <span aria-hidden="true">↗</span></Button></div></FormPanel></Dialog> : null}
    <ModuleToolbar resultLabel={`${files?.totalElements ?? 0} archivos`}><FilterPills active={kind} onChange={(value) => { setKind(value); setPage(0); }} options={[{ value: "all", label: "Todos" }, ...kinds.map((value) => ({ value, label: kindLabel(value) }))]} /><SelectField label="Carpeta" id="file-folder-filter" compact value={folderId} onChange={(value) => { setFolderId(value); setPage(0); }} options={[{ value: "all", label: "Todas las carpetas" }, ...(folders?.content ?? []).map((folder) => ({ value: folder.id, label: folder.name }))]} /></ModuleToolbar>
    {data.loading ? <SkeletonGrid count={4} /> : data.error ? <ErrorState onRetry={data.reload} /> : files?.content.length ? <div className="content-grid files-grid">{files.content.map((file) => <article className="content-card file-card" key={file.id}><div className="content-card-top"><span className="mono-date">{dateLabel(file.uploadedAt, true)}</span><CardActions onEdit={() => startRename(file)} onDelete={() => setPendingDelete(file.id)} /></div><div className="file-card-heading"><VisualTile emoji={kindIcon(file.kind)} label={kindLabel(file.kind)} /><div><span className="file-kind">{kindLabel(file.kind)}</span><span className="file-size">{fileSize(file.sizeBytes)}</span></div></div><h2 title={file.name}>{file.name}</h2><div className="file-folder"><span>▱</span>{file.folder?.name ?? "Sin carpeta"}</div><div className="card-footer"><button type="button" className="eyebrow card-link-button" onClick={() => void download(file)}>DESCARGAR</button><span className="card-arrow">↗</span></div></article>)}</div> : <EmptyState title="No encontramos archivos" description={selectedFolder ? `No hay archivos en ${selectedFolder.name}.` : "Probá con otra carpeta o tipo de archivo. Tu repositorio está listo para recibir el primero."} action="Subir archivo" onAction={() => setUploadComposerOpen(true)} />}
