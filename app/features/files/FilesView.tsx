...
export function FilesView({ focusId }: { focusId?: string | null }) {
  ...
  const handleFile = async () => { if (!selectedFile || !uploadName.trim() || mutation.pending) return; try { await mutation.run(() => api.uploadFile(selectedFile, uploadFolderId || undefined, uploadName.trim())); closeUpload(); invalidateApiQueryCache(); data.reload(); } catch { /* shown in the dialog */ } };
  const createFolder = async () => { const cleanName = folderName.trim(); if (!cleanName || mutation.pending) return; try { const folder = await mutation.run(() => api.createFolder(cleanName)); setFolderName(""); setFolderComposerOpen(false); setFolderId(folder.id); invalidateApiQueryCache(); data.reload(); } catch { /* shown in the dialog */ } };
  const startRename = (file: FileItem) => { setRenameId(file.id); setRenameValue(file.name); setRenameFolderId(file.folder?.id ?? ""); mutation.clearError(); };
  const saveRename = async () => { if (!renameId || !renameValue.trim() || mutation.pending) return; try { await mutation.run(() => api.updateFile(renameId, { name: renameValue.trim(), folderId: renameFolderId || undefined })); setRenameId(null); invalidateApiQueryCache(); data.reload(); } catch { /* shown in the dialog */ } };
  ...
  return <div className="view module-view">
    ...
    {previewFile ? (() => { const kind = getFilePreviewKind(previewFile); return kind && kind !== "image" ? <FilePreviewDialog file={previewFile} kind={kind} onClose={() => setPreviewFile(null)} onDownload={() => void download(previewFile)} /> : null; })() : null}
  </div>;
}
