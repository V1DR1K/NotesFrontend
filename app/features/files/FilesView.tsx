...
export function FilesView() {
  ...
  const prepareFile = (file: File) => { setSelectedFile(file); setUploadDescription(file.name); mutation.clearError(); };
  const closeUpload = () => { setUploadComposerOpen(false); setSelectedFile(null); setUploadDescription(""); setUploadFolderId(""); };
  const handleFile = async () => { if (!selectedFile || !uploadDescription.trim() || mutation.pending) return; try { await mutation.run(() => api.uploadFile(selectedFile, uploadFolderId || undefined, uploadDescription.trim())); closeUpload(); data.reload(); } catch { /* shown in the dialog */ } };
  ...
  const startRename = (file: FileItem) => { setRenameId(file.id); setRenameName(file.name); setRenameDescription(file.description || file.name); mutation.clearError(); };
  const saveRename = async () => { if (!renameId || !renameName.trim() || !renameDescription.trim() || mutation.pending) return; try { await mutation.run(() => api.updateFile(renameId, { name: renameName.trim(), description: renameDescription.trim() })); setRenameId(null); data.reload(); } catch { /* shown in the dialog */ } };
  ...
  return <div className="view module-view">
    <SectionHero section="files" onAction={() => { mutation.clearError(); setUploadComposerOpen(true); }} rightSlot={<div className="storage-card"><div className="storage-card-top"><span className="eyebrow">ESPACIO USADO</span><strong>—</strong></div><div className="storage-bar"><span /></div><small>El uso se calculará con el almacenamiento del backend</small><Button variant="ghost" onClick={() => { mutation.clearError(); setFolderComposerOpen(true); }}>+ Nueva carpeta</Button></div>} />
     ...
     {uploadComposerOpen ? <Dialog ariaLabel="Subir archivo" onClose={closeUpload}><FormPanel title="Subir archivo" description="Elegí un título para encontrarlo después. Por defecto usamos el nombre original." onClose={closeUpload} onSubmit={() => void handleFile()}><div className="form-grid"><SelectField label="Carpeta" id="upload-folder" value={uploadFolderId} onChange={setUploadFolderId} options={[{ value: "", label: "Sin carpeta" }, ...(folders?.content ?? []).map((folder) => ({ value: folder.id, label: folder.name }))]} /><FormField label="Título / descripción" value={uploadDescription} onChange={setUploadDescription} placeholder="Ej. Fotos de vacaciones" /><div className="upload-modal-zone"><div className="dropzone" onClick={() => fileInputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files?.[0]; if (file) prepareFile(file); }} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") fileInputRef.current?.click(); }}><span className="dropzone-icon">↑</span><span><strong>{selectedFile ? selectedFile.name : "Soltá un archivo acá"}</strong><small>{selectedFile ? "Listo para subir" : "o hacé clic para buscarlo en tu dispositivo"}</small></span><span className="dropzone-meta">MULTIPART READY</span></div></div></div>{mutation.error ? <div className="inline-error" role="alert" aria-live="polite">{mutation.error.message || fieldError(mutation.error, "file", "folderId", "description")}</div> : null}<div className="form-actions"><Button variant="quiet" onClick={closeUpload}>Cerrar</Button><Button variant="ghost" onClick={() => fileInputRef.current?.click()}>Elegir archivo</Button><Button type="submit" disabled={mutation.pending || !selectedFile || !uploadDescription.trim()}>{mutation.pending ? "Subiendo..." : "Subir archivo"} <span aria-hidden="true">↗</span></Button></div></FormPanel></Dialog> : null}
    {folderComposerOpen ? <Dialog ariaLabel="Crear carpeta" onClose={() => setFolderComposerOpen(false)}><FormPanel title="Crear carpeta" description="Las carpetas se guardan en tu repositorio personal." onClose={() => setFolderComposerOpen(false)}><div className="form-grid"><FormField label="Nombre de la carpeta" value={folderName} onChange={setFolderName} placeholder="Ej. Documentación" /></div>{mutation.error ? <div className="inline-error" role="alert" aria-live="polite">{mutation.error.message}</div> : null}<div className="form-actions"><Button variant="quiet" onClick={() => setFolderComposerOpen(false)}>Cancelar</Button><Button onClick={() => void createFolder()} disabled={!folderName.trim()}>Crear carpeta <span aria-hidden="true">↗</span></Button></div></FormPanel></Dialog> : null}
     {renameId ? <Dialog ariaLabel="Editar archivo" onClose={() => setRenameId(null)}><FormPanel title="Editar archivo" description="El título es independiente del nombre con el que se descarga el archivo." onClose={() => setRenameId(null)} onSubmit={() => void saveRename()}><div className="form-grid"><FormField label="Título / descripción" value={renameDescription} onChange={setRenameDescription} /><FormField label="Nombre del archivo" value={renameName} onChange={setRenameName} /></div>{mutation.error ? <div className="inline-error" role="alert" aria-live="polite">{mutation.error.message}</div> : null}<div className="form-actions"><Button variant="quiet" onClick={() => setRenameId(null)}>Cancelar</Button><Button type="submit" disabled={!renameName.trim() || !renameDescription.trim() || mutation.pending}>Guardar cambios <span aria-hidden="true">↗</span></Button></div></FormPanel></Dialog> : null}
     ...
     {data.loading ? <SkeletonGrid count={4} /> : data.error ? <ErrorState onRetry={data.reload} /> : allFiles.length ? <div className="content-grid files-grid">{allFiles.map((file) => { const imgIdx = imageIndexMap.get(file.id); return <article className={`content-card file-card ${typeof imgIdx === "number" ? "file-card--image" : ""}`} key={file.id}><div className="content-card-top"><span className="mono-date">{dateLabel(file.uploadedAt, true)}</span><CardActions onEdit={() => startRename(file)} onDelete={() => setPendingDelete(file.id)} /></div>{typeof imgIdx === "number" ? <button type="button" className="file-card-preview" onClick={() => setLightboxIndex(imgIdx)} aria-label={`Ver foto: ${file.description}`}><AuthImage file={file} alt={file.description} loading="lazy" /></button> : <div className="file-card-heading"><VisualTile emoji={kindIcon(file.kind)} label={kindLabel(file.kind)} /><div><span className="file-kind">{kindLabel(file.kind)}</span><span className="file-size">{fileSize(file.sizeBytes)}</span></div></div>}<h2 title={file.description}>{file.description}</h2><div className="file-name" title={file.name}>{file.name}</div><div className="file-folder"><span>▱</span>{file.folder?.name ?? "Sin carpeta"}</div><div className="card-footer">{typeof imgIdx === "number" ? <><button type="button" className="eyebrow card-link-button" onClick={() => setLightboxIndex(imgIdx)}>VER FOTO</button><button type="button" className="eyebrow card-link-button" onClick={() => void download(file)}>DESCARGAR</button></> : <button type="button" className="eyebrow card-link-button" onClick={() => void download(file)}>DESCARGAR</button>}<span className="card-arrow">↗</span></div></article>; })}</div> : <EmptyState title="No encontramos archivos" description={selectedFolder ? `No hay archivos en ${selectedFolder.name}.` : "Probá con otra carpeta, tipo o título. Tu repositorio está listo para recibir el primero."} action="Subir archivo" onAction={() => { mutation.clearError(); setUploadComposerOpen(true); }} />}
